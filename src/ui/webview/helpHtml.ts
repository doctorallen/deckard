...
---</code></pre>
    ...
    <section id="connections">
      ...
      <div class="cards">
        <div class="card"><h3>Related Notes</h3><p>Open Related Notes while editing a saved Markdown note to find entries that may concern the same work.</p><ul><li><strong>Shared tags</strong> are the strongest signal.</li><li><strong>Parent-heading tags</strong> add lighter context to a selected entry.</li><li><strong>Associated tags</strong>, Wiki links, and meaningful keywords provide supporting evidence.</li></ul><p>For example, an entry with both <code>#project/atlas</code> and <code>#follow-up</code> ranks above one connected only through an associated <code>#risk/vendor</code> tag. Repeated association evidence helps with diminishing returns, so indirect connections cannot overtake a complete direct match.</p><p>Hover a tagged heading, line, or task to choose <strong>Show related notes for [entry]</strong>; use <strong>Show whole document</strong> to return. A nested entry with the same tags ranks above its broad parent heading because it is more specific.</p><p>Hover a result percentage for a short explanation, or choose <strong>Debug related notes for [entry]</strong> for selected-tag weights, matched-tag calculations, association paths, links, keywords, and specificity adjustments. Use <strong>Relevance</strong>, <strong>Newest</strong>, <strong>Oldest</strong>, or <strong>Most accessed</strong> to sort results.</p></div>
