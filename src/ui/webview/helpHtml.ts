...
---</code></pre>
    ...
    <section id="connections">
      ...
      <div class="cards">
        <div class="card"><h3>Related Notes</h3><p>With a Markdown note active, the sidebar suggests notes that seem to be about the same work. Shared tags are the clearest match; associated tags, Wiki links, and meaningful shared words can also help Deckard find useful connections. Each result's percentage is a relevance estimate, not a measure of completeness: a higher percentage means Deckard found more or stronger reasons to suggest it. Hover the percentage to see those reasons.                         Hover a tagged entry in Markdown and choose <strong>Debug related notes for [entry]</strong> to open a detailed page that first explains each selected tag&apos;s weight—whether it was written on the entry or inherited from an ancestor—then lists every candidate&apos;s tags, evidence weights, links, keywords, and specificity adjustment.         Candidate tags are evaluated from the displayed entry itself; each candidate has a matched-tag table showing the selected weight, Deckard&apos;s <code>2.00</code> direct-match multiplier, and the resulting contribution.         An association-path table likewise shows every selected-tag-to-candidate-tag connection, its learned strength, and its weighted contribution. It also shows the raw association total and the cap applied at the total selected-tag weight, preventing repeated indirect evidence from overwhelming direct matches. When a broad heading and one of its nested entries match the same tags, Deckard favors the more specific nested entry and labels the broader result's small relevance adjustment. Tagged headings highlight their entire indexed section, while tagged lines and tagged tasks highlight that line; hover one and choose <strong>Show related notes for [entry]</strong> to focus the sidebar on that entry rather than the whole document. Tags on ancestor headings are included as lighter context, so tags written on the selected entry remain most important. Use <strong>Show whole document</strong> in the selected-note header to restore the normal view. Use its sort control to choose <strong>Relevance</strong>, <strong>Newest</strong>, <strong>Oldest</strong>, or <strong>Most accessed</strong>. Date sorts use note modification time; access counts are stored locally. Matching tag chips open their tag overviews. When a Tag Overview is active, the sidebar instead shows its associated tags, while its matching notes omit a redundant 100% score. You can turn off automatic keyword matching in Settings.</p></div>
