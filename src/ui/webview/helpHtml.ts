...
<main>
  ...
  <article>
    ...
    <section id="commands">
      ...
      <ul>
        <li><code>Deckard: Open Dashboard</code> opens workspace totals and the Tasks and Tags Dashboard modes.</li>
...
---</code></pre>
    ...
    <section id="connections">
      ...
      <div class="cards">
        <div class="card"><h3>Related Notes</h3><p>Open Related Notes while editing a saved Markdown note to find entries that may concern the same work.</p><ul><li><strong>Shared tags</strong> are the strongest signal.</li><li><strong>Parent-heading tags</strong> add lighter context to a selected entry.</li><li><strong>Normalized associated tags</strong>, entry Wiki links, and capped section-scoped lexical similarity provide supporting evidence.</li></ul><p>For example, an entry with both <code>#project/atlas</code> and <code>#follow-up</code> ranks above one connected only through an associated <code>#risk/vendor</code> tag. Repeated normalized association evidence helps with diminishing returns, so indirect connections cannot overtake a complete direct match.</p><p>Each result includes its heading path and an inferred daily-note date when available. Hover a tagged heading, line, or task to choose <strong>Show related notes for [entry]</strong>; use <strong>Show whole document</strong> to return. A nested entry with the same tags ranks above its broad parent heading because it is more specific.</p><p>Hover a result percentage for a short explanation, or choose <strong>Debug related notes for [entry]</strong> for selected-tag weights, raw and normalized association support, entry/file links, lexical terms, optional recency, and specificity adjustments. Use <strong>Relevance</strong>, <strong>Newest</strong>, <strong>Oldest</strong>, or <strong>Most accessed</strong> to sort results.</p></div>
