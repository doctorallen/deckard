...
---</code></pre>
    ...
    <section id="connections">
      ...
      <div class="cards">
        ...
        <div class="card"><h3>Notes Graph</h3><p>Run <code>Deckard: Open Notes Graph</code> or select the graph icon in Related Notes, next to the Dashboard icon, to see the whole workspace as a force-directed map. Notes and tasks cluster around hidden tag anchors, with specific tags pulling more strongly than common ones; enable <strong>Show tags</strong> to reveal membership and learned tag-association links. Wiki links and heading nesting remain visible structural links. Scroll to zoom toward the cursor, drag empty space to pan, and drag a dot to rearrange a cluster. Hover a dot to highlight its structural neighbors; select it to overlay the exact top Related Notes results, including shared and associated tags, links, lexical similarity, and optional recency. Hover a sidebar result to identify its graph node, or Cmd/Ctrl-click a graph dot to jump to the source line (a tag dot opens its overview). Use <strong>Filters</strong> to search, restrict by tags, or toggle tasks, tag nodes, and orphans; <strong>Display</strong> adjusts node size, link thickness, and the zoom level where labels appear; <strong>Forces</strong> reshapes the layout with cluster centering, repel strength, link strength, and link distance controls.</p></div>
