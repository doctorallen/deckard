  ...
  <article>
    ...
    <section id="tags">
      ...
      <div class="cards">
        ...
        <div class="card"><h3>Heading relationships</h3><p>Tag Overview infers <strong>Parent tags</strong> and <strong>Child tags</strong> from explicitly tagged headings. It connects a tagged heading to the nearest tagged ancestor, skips untagged intermediate headings, and creates every combination when either heading has multiple tags. Lightweight tag overviews show the relationships with a namespace-collapsible <strong>Tree</strong> view or layered <strong>Graph</strong> view; entity overviews keep them in the compact Related Notes sidebar tree. Selecting a relationship opens the target tag with the current tag applied as a second filter, so only entries carrying both tags are shown; active filters are shown in the page title as <strong>[filter tag] AND [focus tag]</strong>. Use <strong>Clear filter</strong> or reopen the focus tag to return to the full overview. Front-matter tags provide note context but never create synthetic heading relationships.</p></div>
        ...
        <div class="card"><h3>Rename tags</h3><p>Run <code>Deckard: Rename Tag</code> to search indexed tags, select one, and replace it in every matching source occurrence. Enter a complete replacement such as <code>#management/new-name</code>, or enter only a new name to keep the selected tag&apos;s marker and namespace. Source changes are rejected when a note changed after it was indexed.</p></div>
        ...
        <div class="card"><h3>Rank tags</h3><p>To manually order tags, choose <strong>Rank</strong> in the tag sort control, then drag a row above or below another row. You can also right-click a row and choose <strong>Move to top</strong> or <strong>Move to bottom</strong>. Ordering is stored only in Deckard preferences and never reorders Markdown source.</p></div>
      </div>
    </section>
...
---</code></pre>
    ...
    <section id="advanced">
      ...
      <div class="cards">
        ...
        <div class="card"><h3>Heading relationships</h3><p><code>deckard.enableHeadingTagRelationships</code> defaults to <code>true</code> and shows inferred <strong>Parent tags</strong> and <strong>Child tags</strong> on lightweight tag overviews and in the Related Notes sidebar. Entity overviews expose relationships through the sidebar tree instead of the main page. Set the setting to <code>false</code> to hide relationship views without changing ordinary tag indexing or note content.</p></div>
      </div>
    </section>
