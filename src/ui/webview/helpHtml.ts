...
<main>
  ...
  <article>
    ...
    <section id="commands">
      ...
      <ul>
        <li><code>Deckard: Open Dashboard</code> opens workspace totals and the Dashboard's Search and Tags tabs.</li>
...
---</code></pre>
    ...
    <section id="tasks">
      ...
      <div class="cards">
        ...
        <div class="card"><h3>Dashboard navigation</h3><p>The Dashboard opens on the <strong>Search</strong> tab; use its accessible Search/Tags tabs or Left/Right Arrow keys to switch. Its title identifies the active tab, and the View options gear independently sets matching tasks, notes, and tags to one through four columns. Deckard saves the selected tab, searches, and column choices for future Dashboard sessions. <strong>Saved searches</strong> appear above the tabs so they remain available on either. Reopening a saved view shows its name above the matching entity overview title. Tasks have their own page, the Task board.</p></div>
        <div class="card"><h3>Task list</h3><p>Set the Task board's <strong>View options</strong> to <strong>List</strong> to see the searched tasks as rows, with the joined <strong>All</strong>/<strong>Open</strong>/<strong>Done</strong> filter and counts and the <strong>Sort: Rank/Created/Updated</strong> control. Use checkboxes to update the original Markdown task safely; Tag Overviews have the same status filter and checkboxes. With <strong>Rank</strong> selected, drag a task above or below another to set its display order, or right-click it to move it to the top or bottom. Creation-date and update-date sorts are fixed automatically, so dragging is disabled in those modes. Task ranking is stored only in Deckard preferences; it never reorders task lines in your notes.</p></div>
          <div class="card"><h3>Dashboard search</h3><p><strong>Search</strong> lists indexed note entries under the search box, with <strong>Sort: A-Z/Newest created/Recently updated/Most accessed</strong>. Plain words filter the notes as you type; tags and conditions such as <code>is:open</code> apply when you press Enter. <strong>Refine</strong> narrows by tags, task state, due and update dates, and folders, <strong>Matching tasks</strong> lists the tasks the search finds, and <strong>Save</strong>, beside the search box, keeps it as a saved view. Notes use the same full-card presentation and View options format toggle as tag overviews, so you can switch between original Markdown and rendered HTML; the choice is shared with Entity Overview. Select a note entry to open its exact source line; the search is restored when the Dashboard is reopened.</p></div>
        <div class="card"><h3>Browse tags</h3><p>Use <strong>Tags</strong> to browse namespaced entities and lightweight tags together. Search filters the full catalog, and <strong>Namespace</strong> narrows it to one namespace, or to tags without one; favorites remain first, and opening, context-menu actions, and Rank ordering work as they do elsewhere in the Dashboard. Use View options to choose one through four tag columns.</p></div>
      </div>
    </section>
    ...
    <section id="connections">
      ...
      <div class="cards">
        ...
        <div class="card"><h3>Find</h3><p>Run <code>Deckard: Find</code> for results as you type: titles first, then the notes that mention your words, ranked by relevance. Choose <strong>Show all</strong> to see every result on the Dashboard's Search tab.</p></div>
