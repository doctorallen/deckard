  ...
  <article>
    ...
    <section id="tags">
      ...
      <div class="cards">
        ...
        <div class="card"><h3>Heading relationships</h3><p>Tag Overview infers <strong>Parent tags</strong>, <strong>Sibling tags</strong>, and <strong>Child tags</strong> from explicitly tagged headings at any heading level. It connects a tagged heading to the nearest tagged ancestor, skips untagged intermediate headings, and treats tagged headings at the same level under the same structural parent as siblings. It creates every combination when either heading has multiple tags. Lightweight tag overviews show the relationships with a namespace-collapsible <strong>Tree</strong> view or layered <strong>Graph</strong> view; entity overviews keep them in the compact Related Notes sidebar tree. Selecting a relationship opens the target tag with the current tag applied as a second filter, so only entries carrying both tags are shown; active filters are shown in the page title as <strong>[filter tag] AND [focus tag]</strong>. Use <strong>Clear filter</strong> or reopen the focus tag to return to the full overview. Front-matter tags provide note context but never create synthetic heading relationships.</p></div>
...
---</code></pre>
    ...
    <section id="connections">
      ...
      <div class="cards">
        <div class="card"><h3>Related Notes</h3><p>With a Markdown note active, the sidebar ranks saved-note relationships from all shared tags across the note, Wiki links, and significant shared keywords. Use its sort control to choose <strong>Most tags</strong>, <strong>Newest</strong>, <strong>Oldest</strong>, or <strong>Most accessed</strong>. Date sorts use note modification time; access counts are stored locally. Matching tag chips open their tag overviews. When a Tag Overview is active, the sidebar instead shows its relationship tree. You can turn off automatic keyword matching in Settings.</p></div>
    ...
    <section id="advanced">
      ...
      <div class="cards">
        ...
        <div class="card"><h3>Heading relationships</h3><p><code>deckard.enableHeadingTagRelationships</code> defaults to <code>true</code> and shows inferred <strong>Parent tags</strong>, <strong>Sibling tags</strong>, and <strong>Child tags</strong> on lightweight tag overviews and in the Related Notes sidebar. Entity overviews expose relationships through the sidebar tree instead of the main page. Set the setting to <code>false</code> to hide relationship views without changing ordinary tag indexing or note content.</p></div>
      </div>
    </section>
