...
${getQueryEditorScript()}
  ...
  function renderHub(isQueryView, filterTags) {
    ...
    if (isQueryView || filterTags.length || !state.tag || (isRefinement() && state.query.text)) return '';
    const hub = state.hub;
    if (!hub) {
  ...
  function render() {
    ...
    const saveFilterControl = filterTags.length || isQueryView || (isRefinement() && state.query.text)
    ...
    document.getElementById('app').innerHTML = '<header><div><div class="overview-eyebrow"><p class="eyebrow">' + eyebrow + '</p>' + saveFilterControl + '</div>' + savedViewName + '<h1 aria-label="' + escapeHtml(titleAriaLabel) + '">' + titleHtml + '</h1>' + entityMeta + '</div>' + headerControls + '</header>' + editor.renderBar() + editor.renderFacets() + hub + layoutContent;
    filterOverviewEntries('notes');
    filterOverviewEntries('tasks');
    editor.afterRender();
  }
