...
The script needs the `code` command on `PATH` and a free CDP port. It uses CDP only to discover the workbench target and capture the screenshot. It chooses a free port by default; set `DECKARD_CDP_PORT` to override it. Set `DECKARD_SCREENSHOT_THEME` to `replicant`, `oblivion`, or `lcars`, and set `DECKARD_SCREENSHOT_OUTPUT` to choose a different output path. Set `DECKARD_SCREENSHOT_HOVER` to a selector inside the view's webview to photograph it hovered, which is how a hover state is checked, and `DECKARD_SCREENSHOT_CLICK` to selectors, comma separated, clicked in order first, for a view that only shows something once it has been used. On failure, temporary host paths are printed and retained for diagnosis.
