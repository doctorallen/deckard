...
- Tag Overview infers parent, child, and sibling tag relationships from explicitly tagged headings at any heading level. A tagged heading is related to the nearest tagged ancestor heading, even when untagged headings appear between them or the note starts below H1; headings with tags at the same level under the same structural parent are siblings. Front-matter tags do not create these relationships.
...
- switch inferred **Parent tags**, **Sibling tags**, and **Child tags** between a namespace-collapsible **Tree** view and a layered **Graph** view on lightweight tag overviews; all remain clickable, and repeated relationship pairs show a compact count; and
- follow a parent, sibling, or child relationship into the target overview with the current tag applied as a second filter, so only entries carrying both tags are shown; active relationship filters are folded into the page title as **[filter tag] AND [focus tag]**; use **Clear filter** or reopen the focus tag to return to the full overview; and
...
Set `deckard.enableHeadingTagRelationships` to `false` when you want to hide the inferred Parent tags, Sibling tags, and Child tags views, including the sidebar tree, while keeping ordinary tag indexing and note content unchanged.
...
| `deckard.enableHeadingTagRelationships` | `true` | Shows inferred **Parent tags**, **Sibling tags**, and **Child tags** in Tag Overview, with Tree and Graph views. Disable it to hide the relationship workspace without changing indexed tags or note content. |
