...
- ~~The Dashboard's Notes tab is still sent, and draws, every note on each save: about 9.9 MB at 940 notes in the Markdown view. Page or virtualize that list if it shows up in Deckard's log.~~ Done, on search pages, where the list moved: each list is paged, 30 results to a page by default and at most 200, so a page's payload is about 0.08 MB in the source view at the default and 0.54 MB at the largest, whatever the workspace holds, rather than 4.3 MB at 940 notes and growing with it. Building the snapshot still costs a pass over every section, about 1.1 s at 5,000 notes; that is the evaluator, not the channel.
